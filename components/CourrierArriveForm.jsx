      ...
      <form className="bg-surface border border-primary/20 rounded-xl shadow-lg p-6 space-y-4" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="datetime-local" className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:border-primary focus:ring-1 focus:ring-primary" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} required aria-required="true" />
          </div>
          <div>
            <label className="block text-sm mb-1 text-main font-medium">{t('expediteur')}</label>
            <select className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:border-primary focus:ring-1 focus:ring-primary" value={form.expediteur} onChange={e => setForm({ ...form, expediteur: e.target.value })} required aria-required="true">
          ...
          <div>
            ...
            <select className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:border-primary focus:ring-1 focus:ring-primary" value={form.statut} onChange={e => setForm({ ...form, statut: e.target.value })}>
              {STATUTS.map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm mb-1 text-main font-medium">{t('planif')}</label>
            <input type="datetime-local" className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:border-primary focus:ring-1 focus:ring-primary" value={form.planif} onChange={e => setForm({ ...form, planif: e.target.value })} />
          </div>
          <div>
            <label className="block text-sm mb-1 text-main font-medium">{t('delai')}</label>
            <input type="datetime-local" className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:border-primary focus:ring-1 focus:ring-primary" value={form.delai} onChange={e => setForm({ ...form, delai: e.target.value })} />
          </div>
        </div>
      ...
      <div className="bg-surface border border-primary/20 rounded-xl shadow-lg p-6 mt-8">
        <div className="flex flex-col md:flex-row gap-2 md:items-center mb-4">
          <input
            ...
            className="bg-gray-700 text-white rounded-lg px-3 py-2 w-full md:w-1/2 border border-gray-600 focus:border-primary focus:ring-1 focus:ring-primary"
            aria-label={t('searchBySubject')}
          />
          <select
            value={statutFilter}
            onChange={e => setStatutFilter(e.target.value)}
            className="bg-gray-700 text-white rounded-lg px-3 py-2 w-full md:w-1/4 border border-gray-600 focus:border-primary focus:ring-1 focus:ring-primary"
