  ...
  return (
    <div className="space-y-8">
      <form className="bg-gray-900 dark:bg-surface rounded-xl shadow-lg p-6 space-y-4" onSubmit={handleSubmit}>
        <h2 className="text-2xl font-semibold mb-2">{t('registerInMail')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm mb-1">{t('receiveDate')}</label>
            <input type="datetime-local" className="w-full bg-muted/30 text-gray-300 rounded px-3 py-2" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} required aria-required="true" />
          </div>
          <div>
            <label className="block text-sm mb-1">{t('expediteur')}</label>
            <select className="w-full bg-muted/30 text-gray-300 rounded px-3 py-2" value={form.expediteur} onChange={e => setForm({ ...form, expediteur: e.target.value })} required aria-required="true">
          ...
          <div>
            <label className="block text-sm mb-1">{t('destinataire')}</label>
          ...
          <div>
            <label className="block text-sm mb-1">{t('statut')}</label>
            <select className="w-full bg-muted/30 text-gray-300 rounded px-3 py-2" value={form.statut} onChange={e => setForm({ ...form, statut: e.target.value })}>
              {STATUTS.map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm mb-1">{t('planif')}</label>
            <input type="datetime-local" className="w-full bg-muted/30 text-gray-300 rounded px-3 py-2" value={form.planif} onChange={e => setForm({ ...form, planif: e.target.value })} />
          </div>
          <div>
            <label className="block text-sm mb-1">{t('delai')}</label>
            <input type="datetime-local" className="w-full bg-muted/30 text-gray-300 rounded px-3 py-2" value={form.delai} onChange={e => setForm({ ...form, delai: e.target.value })} />
          </div>
        </div>
        <div className="flex gap-2 mt-4">
          ...
          <button type="button" className="w-full bg-gray-700 hover:bg-gray-600 text-white font-semibold py-2 rounded transition" onClick={() => setForm({ ...form, objet: '', fichiers: [], planif: '', delai: '' })}>{t('reset')}</button>
        </div>
        {message && (
